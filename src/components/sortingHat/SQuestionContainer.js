...
export default class SQuestionContainer extends React.Component {
  ...
  imageArray = [
    ...
    "https://www.dailydot.com/wp-content/uploads/6f3/1f/pottermore-ron.jpg", 
  ...
  render() {
    ...
    return question ? (
      <div className="sorting_question" style={{ backgroundImage: `url(${this.imageArray[this.state.currentQuestion.questionNumber - 1]})`}}>
        <Card  id="card" style={{ "font-family":"'Lilita One', cursive", "font-size":"20px", width: `${this.widthArray[this.state.currentQuestion.questionNumber - 1]}` , opacity: "0.9"}}>
        <Card.Body>
        <Card.Title>
          <h2 style={{ "font-family":"'Lilita One', cursive"}}>Question {question.number}: {question.question}</h2>
        </Card.Title>
        </Card.Body>
